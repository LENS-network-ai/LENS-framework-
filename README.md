...
## Parameter Guidelines for L0 Regularization

- **lambda_reg**: Controls overall regularization strength
  - Higher values → More aggressive pruning
  - Lower values → Less pruning, better accuracy

- **l0_beta**: Controls the 'hardness' of binary decisions
  - Higher values → Smoother, less binary decisions
  - Lower values → Sharper, more binary decisions

- **l0_gamma & l0_zeta**: Control the range of the hard sigmoid
  - Default [-0.1, 1.1] usually works well
  - Wider range increases the binary nature of weights

- **initial_temp**: Controls randomness in training
  - Higher values → More exploration of different edge patterns
  - Lower values → More deterministic edge selection
